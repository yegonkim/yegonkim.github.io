...
I think this makes policy search generally more robust in multiple ways. With real-world imperfect optimizers, a modular design can lead to errors compounding, for instance an imperfect world model being prone to reward hacking by the planner. Another interesting case is embedded agency, where doing a policy search, akin to updateless decision theory, can avoid multiple failures of traditional decision theories.
Moreover, processes at various scales can be regarded as policy search, from evolution and AI research, to the optimization being done by a policy gradient algorithm.
